...
import RolModel from 'Frontend/generated/com/yefersoncm/app/data/entity/RolModel';
...
export class RolesView extends View {
  ...
  private binder = new Binder<Rol, RolModel>(this, RolModel);
  ...
  private async getGridData(
    params: GridDataProviderParams<Rol>,
    callback: GridDataProviderCallback<Rol | undefined>
  ) {
    const sort: Sort = {
    ...
    const data = await RolesEndpoint.list({ pageNumber: params.page, pageSize: params.pageSize, sort });
    callback(data);
  }
  ...
  async connectedCallback() {
    ...
    this.gridSize = (await RolesEndpoint.count()) ?? 0;
  }

  private async itemSelected(event: CustomEvent) {
    const item: Rol = event.detail.value as Rol;
    this.grid.selectedItems = item ? [item] : [];

    if (item) {
      const fromBackend = await RolesEndpoint.get(item.id!);
  ...
  private async save() {
    try {
      ...
      await this.binder.submitTo(RolesEndpoint.update);
      if (isNew) {
        // We added a new item
        this.gridSize++;
      }
      this.clearForm();
      this.refreshGrid();
      Notification.show(`Rol guardado correctament.`,  { theme: 'primary', position: 'bottom-start' });
    } catch (error: any) {
